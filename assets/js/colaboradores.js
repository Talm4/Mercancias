...
function stateChip(registros, hoy) {
  const global = estadosPorPersona(registros, hoy).global?.estado;
  if (!global) return '<span class="est-chip soft-gray">Sin registro</span>';
  const c = coloresEstadoCapacitacion(global.estado);
  return `<span class="est-chip ${c.soft}"><i class="fa-solid ${global.icono}"></i>${escapeHtml(global.etiqueta)}</span>`;
}
...
export function renderColaboradores(s) {
  ...
  document.getElementById("personasTbody").innerHTML = visible.length ? visible.map(p => {
    ...
    return `<tr><td><span class="person-link" data-open-perfil="${escapeHtml(p.key)}">${escapeHtml(p.NOMBRES)}</span><br><small class="text-muted">${escapeHtml(p.CARGO || "Sin cargo")}</small></td><td class="mono">${escapeHtml(p.ID)}</td><td>${escapeHtml(p.BASE || "—")}</td><td>${p.totalCursos}</td><td><strong>${pct}%</strong> <small class="text-muted">${p.asistencias}/${p.inasistencias}</small></td><td>${stateChip(p.registros, s.estadoHoy)}</td><td class="mono">${formatFechaDisplay(p.ultimaFecha)}</td></tr>`;
  }).join("") : '<tr><td colspan="7" class="empty-cell">No hay personas para este filtro.</td></tr>';
